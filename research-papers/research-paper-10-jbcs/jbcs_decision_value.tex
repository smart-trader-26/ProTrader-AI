% =============================================================================
% Journal of the Brazilian Computer Society (JBCS) submission.
% "Accounting for Decision Value: Calibration and Selection over Prediction
%  in a Deployed Equity-Forecasting System."
%
% Built on the official JBCS template (sbc2023 class, Viterbo et al.).
% COMPILE WITH XeLaTeX (template requirement; uses fontspec + Times New Roman):
%     xelatex jbcs_decision_value
%     bibtex  jbcs_decision_value
%     xelatex jbcs_decision_value
%     xelatex jbcs_decision_value
% Every quantitative result is a measured statistic from the authors' deployed
% system or its live append-only ledger; figures are vector (pgfplots/TikZ).
% =============================================================================
...
\providecommand{\faEnvelopeO}{\faEnvelope}
...
% ---- additions used by this manuscript --------------------------------------
...
% sbc2023.cls uses \affilfont in \affil but leaves its definition commented out;
% supply it so the author block compiles cleanly.
\providecommand{\affilfont}{\footnotesize\raggedright}
...
\jid{JBCS}
\jtitle{Journal of the Brazilian Computer Society, 2026, 32:1, }
\doi{10.5753/jbcs.2026.XXXXXX}
\copyrightstatement{This work is licensed under a Creative Commons Attribution 4.0 International License}
\jyear{2026}

\title[Accounting for Decision Value]{Accounting for Decision Value: Calibration and Selection over Prediction in a Deployed Equity-Forecasting System}

\author[Pardeshi and Deshmukh 2026]{
\affil{\textbf{Anandkumar Pardeshi}~\href{https://orcid.org/0000-0002-1825-0097}{\textcolor{orcidlogo}{\aiOrcid}}~\textcolor{blue}{\faEnvelopeO}~~[~\textbf{Fr.\ C.\ Rodrigues Institute of Technology, University of Mumbai}~|\href{mailto:anand.pardeshi@fcrit.ac.in}{~\textbf{\textit{anand.pardeshi@fcrit.ac.in}}}~]}

\affil{\textbf{Sujata Deshmukh}~\href{https://orcid.org/0000-0001-5109-3700}{\textcolor{orcidlogo}{\aiOrcid}}~~[~\textbf{Fr.\ C.\ Rodrigues Institute of Technology, University of Mumbai}~|\href{mailto:sujata.deshmukh@fcrit.ac.in}{~\textbf{\textit{sujata.deshmukh@fcrit.ac.in}}}~]}
}
...
\begin{mail}
Department of Computer Science and Engineering, Fr.\ C.\ Rodrigues Institute of Technology, University of Mumbai, Agnel Technical Education Complex, Sector 9-A, Vashi, Navi Mumbai, 400703, India.
\end{mail}
...
\begin{abstract}
\textbf{Abstract}
\noindent Most machine-learning work on equity markets is graded on predictive accuracy or area under the ROC curve, and most of it fails to beat a trivial drift baseline once costs and look-ahead are handled honestly. We argue that accuracy measures the wrong thing. What a deployed system delivers is \emph{decision value}: the quality of the small set of positions it actually takes, weighed against the far larger set it stays out of. We make this precise by extending the classical calibration--refinement and reliability--resolution decompositions of forecast quality to the abstaining, cost-aware decision setting, and we prove a tail--AUC decoupling result: a one-sided selective rule can earn a positive edge in its high-conviction tail while global AUC sits at or below one half, so AUC and accuracy systematically misreport what such a system is worth. We then \emph{account} for the value of a real deployed system on the Indian National Stock Exchange across its whole pipeline: multi-source features, a hybrid gradient-boosted/recurrent ensemble, language-model news scoring, post-hoc calibration, a conviction gate, and an append-only audit ledger. The numbers are unforgiving. On 621 resolved forecasts from the live ledger, nominal 90\% price intervals covered only 70.7\% of outcomes, so deployed confidence was overstated and self-correcting it was nobody's job. A three-stage recalibration that first repaired a stacking domain mismatch cut expected calibration error from 0.35 to 0.05 without moving accuracy. Single-name next-period direction was unpredictable beyond drift (50.1\% out-of-sample, ranking AUC near 0.49). Yet a conviction gate that abstains on roughly 94\% of names lifted the 30-day hit rate to 60.6\% against a 58.0\% base (95\% interval 58.8--62.4), positive in four of five test years. Selection produces the realized edge. Calibration is what makes the gate trustworthy enough to act on. The predictive model adds almost no ranking alpha. The practitioner's lesson follows: instrument the uncertainty layer and report precision at coverage, rather than optimising a number the deployment will not pay for.
\end{abstract}

\begin{keywords}
Decision value, Calibration, Selective prediction, Probabilistic forecasting, Financial machine learning, Model evaluation, Abstention
\end{keywords}
...
\citep{sezer2020,jiang2021}. And yet the people who actually run money are not
...
partition of \citep{degroot1983} and the reliability--resolution partition
of \citep{murphy1973}, and it makes ``where does the value come from'' a question with a
measurable answer.
...
memory networks \citep{fischer2018}, gradient-boosted trees and random forests
for statistical arbitrage \citep{krauss2017}, and large cross-sectional studies of
machine-learning asset pricing \citep{gu2020} are representative, and two
systematic reviews map the territory \citep{sezer2020,jiang2021}. The dominant
...
\citep{bailey2017}, the cross-section of ``discovered'' predictors is riddled with
multiple-testing artefacts \citep{harvey2016}, and a careful treatment of leakage,
costs and capacity removes most reported edges \citep{lopezdeprado2018}. Our paper sits
...
Modern classifiers are frequently overconfident \citep{guo2017}. The standard
post-hoc remedies are Platt scaling \citep{platt1999} and isotonic regression
\citep{zadrozny2002}, with reliability assessed by the expected calibration
error \citep{naeini2015} and the Brier score \citep{brier1950,niculescu2005}. The foundational decompositions of forecast quality are older and,
for our purposes, central: \citet{murphy1973} partitions the Brier score into reliability,
resolution and uncertainty, and \citet{degroot1983} partition forecaster
quality into calibration and refinement. \citet{gneiting2007} crystallise the
...
\citep{wolpert1992} is that a calibrator fitted on one stage's scores and applied to
another silently fails. We diagnose this failure in Section~\ref{sec:cal}.

\noindent\textbf{Selective prediction and abstention.}
Allowing a model to decline to answer is an old idea \citep{chow1970} with a modern
theory of risk--coverage trade-offs \citep{elyaniv2010,geifman2017} and a distribution-free cousin in conformal prediction \citep{vovk2005,angelopoulos2021}, including variants built for the non-stationary
coverage failures we observe \citep{gibbs2021}. This community measures
...
\citep{tetlock2007,antweiler2004}, that finance-specific dictionaries matter
because general ones mislabel domain terms is the lesson of \citep{loughran2011}, and supervised or pre-trained language models now dominate sentiment
extraction \citep{devlin2019,ke2019}. Whether instruction-tuned models
can read financial news and anticipate moves is under active study \citep{lopezlira2023}. Our system uses a language model as one feature source; we are careful,
...
\emph{useful}: what a decision-maker gains by acting on it. \citet{murphy1993}
...
become a usable signal. \citet{granger2000} sharpened the
...
when honest probabilities are rewarded \citep{gneitingraftery2007}, and in finance the
realised worth of a strategy is usually read off a risk-adjusted return such as the
Sharpe ratio \citep{sharpe1994}. Decision value belongs to this lineage. It is a value
...
in \citep{murphy1973}. The first term, the calibration gap, is how wrong that claim turns
...
result. \citet{murphy1973} split the Brier score of a probabilistic forecaster into
three pieces,
\begin{equation}
\label{eq:murphy}
...
$\tfrac1N\sum_i(\Pup_i-y_i)^2$ \citep{brier1950}. The calibration stage applies a learned
...
stacked-generalisation trap \citep{wolpert1992}: an isotonic calibrator had been fitted on
...
\citep{platt1999} on a held-out fold and apply it to both the test rows and the live row.
...
prediction for liquid names \citep{fama1970,malkiel2003}, and it is step three of the
...
because their efficiency drives the ranking term toward zero \citep{fama1970,malkiel2003} and their outcomes are public and unforgiving, but the structure recurs
...
both easy to overfit in backtests \citep{bailey2017,harvey2016} and
largely irrelevant to a system that acts on its tail. Reporting precision at coverage,
as the selective-prediction community does \citep{geifman2017,elyaniv2010}, aligns the metric with the deployment. The decision-value
...
The conviction gate is an instance of selective classification \citep{chow1970,geifman2017}; what the decision-value account adds is the attribution,
...
built to address; adaptive conformal inference \citep{gibbs2021,angelopoulos2021} is designed for this kind of non-stationary
...
of Technology, University of Mumbai, for computing support.
...
\begin{contributions}
AP conceived the decision-value accounting framework, built and operated the deployed
system, performed the experiments, and drafted the manuscript. SD supervised the work,
contributed to the evaluation design and the analysis, and reviewed and edited the
manuscript. Both authors read and approved the final manuscript. Author contributions
follow the CRediT taxonomy (\url{https://credit.niso.org/}).
\end{contributions}

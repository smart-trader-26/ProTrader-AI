"""
Study 5, step 0 -- build and cache the full real feature panel once
(technical + VIX + ALL real macro tickers from data/macro.py) so the
follow-up sweep scripts can test many (horizon, source-set) combinations
without re-downloading from yfinance every time.

This is exploratory infrastructure for testing whether additional real,
differentiated sources or different horizons improve on Study 4's result --
not paper content. Nothing here is written into the paper; it only feeds
analysis/results/study5_*.json summaries for review.
"""
...
CACHE = HERE / "results" / "study5_panel_cache.pkl"

"""Build the AECE manuscript.

Every quantitative statement in the text is interpolated from the result
files written by steps 4-6, so the prose cannot drift away from the
analysis.  Citations are written as [[tag]] and renumbered in order of
first appearance, which is what the journal requires.
"""
...
from docbuild import (I_ABSTRACT, I_AFF1, I_AFF2, I_AUTHORS,  # noqa: E402
                      I_BODY_END, I_BODY_START, I_EMAIL, I_INDEX,
                      I_TITLE_A, I_TITLE_B, Builder, clear_body,
                      load_template, roman, set_text,
                      strip_template_remnants, to_doc)
from refs import REFS                              # noqa: E402

TITLE_A = "An Attenuation-Corrected Matched Filter with Closed-Loop Gain"
TITLE_B = "Design for News-Sentiment Aggregation in Equity Forecasting"
AUTHORS = "Anandkumar PARDESHI{^1}, Sujata DESHMUKH{^2}"
AFF1 = ("{^1}Department of Computer Science and Engineering, Fr. C. Rodrigues "
        "Institute of Technology, University of Mumbai, Vashi, Navi Mumbai, "
        "400703, India")
AFF2 = ("{^2}Department of Information Technology, Fr. C. Rodrigues Institute "
        "of Technology, University of Mumbai, Bandra, Mumbai, 400050, India")
EMAIL = "anand.pardeshi@fcrit.ac.in"
INDEX_TERMS = ("adaptive filters, least mean squares methods, matched "
...
def filter_reading(R: Res) -> str:
    """Interpretation of Table III that follows from the identified kernels."""
    bits = []
    cr = compare(R.f, R, 1)
    cv = compare(R.fv, R, 1)
    ...
    if cv:
        ...
        fam = d[d["filter"].str.startswith(("uniform", "cwin", "exp"))
                | (d["filter"] == "latest")]["ic"]
        bits.append(
            "The volatility block shows the opposite regime. The response "
...
def loop_reading(R: Res) -> str:
    ...
    if clv:
        ...
        if ic_star is not None and best_static is not None:
            rel_txt = (
                "above" if ic_star > best_static else "below")
            bits.append(
                ...
                f"{rel_txt} every static filter in Table III including "
                ...
                "region of Fig. 6 rather than by search.")
    if cl:
        bits.append(
...
def content(R: Res, figdir: str):
    """Return the ordered list of body items."""
    it = []
    A = it.append

    k, kv, cl = R.k, R.kv, R.cl
    n_obs = k.get("n_obs", 0)
    ...
    def lam(name):
        d = rel.get(name, {})
        return d.get("lambda_median")

    lam_fb, lam_lm, lam_vd = lam("finbert"), lam("lm"), lam("vader")
    corr = rel.get("_corr", {})
    ...
    A(("text",
       ...
       "level. Regressors measured with error produce attenuated and, in "
       "the multi-lag case, reshaped coefficients [[fuller]], "
       "[[carroll]]."))
    A(("text",
       ...
       "measurement noise shrinks it. This paper separates them. The key "
       "observation is that several sentiment scorers built from "
    ...
    A(("text",
       ...
       "session "
       "return and next-session realised volatility, and is shown to "
       "recover a short kernel for the first and a long one for the "
       "second."))
    ...
    A(("text",
       ...
       "seldom posed in those terms. Recent work in this journal applies "
       "learned encoders and adaptive numerical schemes to other domains "
       "[[aece_dga]], [[aece_mpnn]], [[aece_quant]]. The present paper "
       "supplies the missing link for the sentiment pipeline."))

    # -------------------------------------------------- III. Signal model
    A(("head", "Signal Model and Identification"))
    A(("subhead", "A. Measurement model"))
    A(("text",
    ...
    A(("text",
       ...
       "training corpus."))
    A(("subhead", "B. Identification from cross-scorer covariance"))
    A(("text",
    ...
    # ------------------------------------------------- IV. Kernel
    A(("head", "Response-Kernel Identification"))
    A(("text",
    ...
    A(("eq", M.d(M.hat(M.r("g")), M.up(" = "),
                 M.sup(M.paren(M.d(M.sub(M.r("R"), M.r("mm")), M.up(" + "),
                                   ...
                                   M.r("D"))), M.up("-1")), M.r("c"))))
    A(("text",
       "In (6) and (7), {i:D} is the second-difference operator, so the penalty is on the "
    ...
    # ------------------------------------------------- V. Matched filter
    A(("head", "Matched Filtering of the Aggregation Step"))
    A(("text",
    ...
    A(("text",
       "so the optimal aggregation rule (8) is the matched filter for the identified kernel whitened by "
    ...
    # ------------------------------------------------- VI. Closed loop
    A(("head", "Closed-Loop Gain Design"))
    A(("text",
    ...
    # ------------------------------------------------- VII. Setup
    A(("head", "Data and Experimental Protocol"))
    A(("text",
       ...
       "No text is generated, augmented or simulated at any point. Prices "
    ...
    A(("text",
       ...
       "test of equal squared error [[dm]]. Table " + roman(1) + " "
    ...
    A(("head", "Results"))
    A(("text",
       "Fig. 1 summarises the path from headlines to a deployed filter: "
       "the cross-scorer covariance strips the scorer noise out of the "
       "autocovariance, the kernel follows by deconvolution, and the "
       "matched filter derived from it is closed with a feedback path."))
    ...
    A(("subhead", "A. How noisy are sentiment scorers"))
    A(("text",
       "Table " + roman(2) + " reports the agreement between the scorers "
       "and the reliability implied by it. The scorers correlate weakly "
       "with one another, and the resulting reliability ratios are far "
       f"below one: the transformer scorer carries about "
    ...
    A(("text",
       ...
       "reliability must rise with news intensity, and Fig. 2 confirms "
    ...
    A(("subhead", "B. Response kernels for return and volatility"))
    A(("text",
       "Fig. 3 shows the lag profile, the conventional deconvolution and "
       "the noise-corrected kernel for the return response, and Fig. 4 "
       "the same for volatility. The two responses have qualitatively "
       "different shapes, which is the clearest evidence that the "
       "procedure is measuring something real rather than imposing a "
       "form."))
    ...
    A(("text",
       "The noise correction raises the magnitude of the kernel "
       "throughout, as attenuation theory requires, and it does so "
       "unevenly across lags, confirming that measurement error reshapes "
       "rather than merely rescales the estimated response."))

    A(("subhead", "C. Filters out of sample"))
    A(("text",
       "Table " + roman(3) + " compares aggregation filters on the "
    ...
    A(("text",
       "Fig. 5 shows the same comparison for the volatility response as "
    ...
    A(("subhead", "D. Loop gain against its prediction"))
    A(("text",
    ...
    A(("text",
       "Fig. 6 plots the held-out error against the gain. The error is "
       "flat over a wide range of small gains and rises sharply as the "
       "stability bound is approached, which is the behaviour the "
       "mean-square analysis predicts, and the predicted gain falls "
    ...
    A(("text",
       ...
       "II. The deployed filter is an inner product of length {i:K} per "
    ...
    A(("text",
       ...
       "the instrument more than the persistence of the news."))
    A(("text",
       ...
       "rather than inherited as a convention."))
...
def main() -> None:
    ...
    set_text(ps[I_TITLE_A], TITLE_A)
    set_text(ps[I_TITLE_B], TITLE_B)
    set_text(ps[I_AUTHORS], AUTHORS)
    set_text(ps[I_AFF1], AFF1)
    set_text(ps[I_AFF2], AFF2)
    set_text(ps[I_EMAIL], EMAIL)

    abstract = build_abstract(R)
    set_text(ps[I_ABSTRACT], abstract, bold_lead="Abstract—")
    set_text(ps[I_INDEX], INDEX_TERMS, bold_lead="Index Terms—")
    ...
    clear_body(doc, I_BODY_START, I_BODY_END)
    b = Builder(doc, I_BODY_START)
    ...
    b.refhead("Conflict of Interest")
    b.text("The authors declare that the research was conducted in the "
           "absence of any commercial or financial relationships that "
           "could be construed as a potential conflict of interest.")
    b.refhead("Publisher's Note")
    b.text("All claims expressed in this article are solely those of the "
           "authors and do not necessarily represent those of their "
           "affiliated organizations, or those of the publisher, the "
           "editor and the reviewers. Any statements, claims, "
           "performances and results are not guaranteed or endorsed by "
           "the publisher.")
    b.refhead("References")
    # the References style numbers entries automatically; adding "[i]" here
    ...
    if len(order) < 25:
        print("!! FEWER THAN 25 REFERENCES")

    n = strip_template_remnants(doc)
    print(f"removed {n} template remnant paragraphs")
    doc.save(args.out)
    print(f"saved {args.out}")


def build_abstract(R: Res) -> str:
    """At most 200 words, no citations, plain English characters only."""
    k = R.k
    rel = k.get("reliability", {})
...
def build_tables(R: Res) -> dict:
    ...
    # ---- Table: scorer agreement and reliability

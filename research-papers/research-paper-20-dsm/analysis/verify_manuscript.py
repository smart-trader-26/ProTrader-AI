...
TEX = ROOT / "dsm_validity_screen.tex"
LOG = ROOT / "dsm_validity_screen_BLIND.log"
MACROS = [ROOT / "macros.tex", ROOT / "macros_cost.tex"]
...
def main() -> None:
    ...
    # ---- abstract ----------------------------------------------------------
    # elsarticle takes the abstract as an environment, the Springer class as an
    # argument; the manuscript may be built for either, so both are accepted.
    m = (re.search(r"\\abstract\{(.+?)\}\s*\n\s*\\keywords", tex, re.S)
         or re.search(r"\\begin\{abstract\}(.+?)\\end\{abstract\}", tex, re.S))
    if m:
    ...
    # ---- references --------------------------------------------------------
    bbl = ROOT / "dsm_validity_screen_BLIND.bbl"
    if bbl.exists():

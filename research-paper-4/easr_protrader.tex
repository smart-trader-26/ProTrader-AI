...
large pre-trained language models \cite{r1,r8}. Despite measurable progress, three
...
are shaped by news in both English and Hindi. Nti et al.\ \cite{r2}, Long
et al.\ \cite{r3}, and Fu and Zhang \cite{r4} demonstrated that fusing heterogeneous modalities lifts
accuracy above single-source baselines, but none of these studies addresses the Indian
...
\textbf{Stream 4 (Macro State):} India VIX is encoded as a normalised level
($(V\!-\!15)/25$, clipped to $[\pm1]$) and 1-day change (clipped to $[\pm0.50]$);
INR/USD, front-month Brent crude, and gold spot are retrieved via yfinance with
1-day and 5-day log-returns clipped to $[\pm0.20]$ and $[\pm0.40]$.

\subsection{Feature Engineering (RO1, RO2)}

From the four streams a joint feature vector $\mathbf{x}_{t}\!\in\!\mathbb{R}^{32}$
is constructed. The 18 price-volume technical features are listed in
...
Pattern      & Pattern\_Target\_Dist (signed, norm.)      & variable \\
Sentiment    & FinBERT polarity $s_t$ (wtd.\ mean)       & 1d \\
Inst.\ Flow  & FII\_Net, DII\_Net, FII\_5D\_Avg,
               DII\_5D\_Avg (lagged, norm.)              & 1d, 5d \\
VIX          & VIX\_Norm, VIX\_Chg (India VIX)          & 1d \\
Macro        & INR\_Ret, Crude\_Ret, Gold\_Ret           & 1d, 5d \\
\bottomrule
\end{tabular}
\end{table}
...
Headlines from Stream~2 are scored by a FinBERT pipeline \cite{r12,r31}
...
Calibration quality is measured by ECE over $M=10$ equal-frequency bins and the
...
procedure of Romano et al.\ \cite{r26}, grounded in the conformal prediction
framework of Vovk et al.\ \cite{r18} and surveyed in Angelopoulos and Bates \cite{r33}. Two quantile XGBoost regressors are
...
\textbf{Hurst regime detection.} The Hurst exponent \cite{r19} is estimated from the
...
\textbf{Granger-causality.} Following Granger \cite{r30}, for each of the 50 tickers we test whether lagged
...
$p$-values are combined via Fisher's method. The sum starts at $j=0$ because
$s_t$ is scored before the 15:30~IST close and is therefore available without
look-ahead as a predictor of next-day returns $r_{t+1}$.
...
(iii)~two-layer LSTM on 18 technical features; (iv)~XGBoost (18 features, no
...
($M=10$ bins), annualised Sharpe ($\sqrt{252}\,\bar{r}/\hat\sigma_r$),
...
\midrule
Buy-and-hold (NIFTY~50)   & 51.2\% & ---   & ---    & 0.78 & $-23.8\%$ & --- \\
...
\begin{axis}[
  ...
  yticklabels={OBV Slope, Vol.\ Ratio, MACD, VIX Ret., BB\,\%B,
...
\addplot[fill=blue!30,draw=blue!55] coordinates {
  (0.047,0)(0.059,1)(0.068,2)(0.076,3)(0.087,4)
...
institutional-flow channel. Removing sentiment costs 0.27~Sharpe but only 1.6~pp of ECE, revealing that
the LLM channel carries distinct \emph{discriminative} information; removing
FII/DII costs 0.16~Sharpe but 0.5~pp ECE, indicating the institutional channel
serves calibration more than raw direction.
...
guarantee given serial dependence \cite{r27,r28}.

\textbf{RO3 --- Explainability and Temporal Dynamics.} XGBoost-component SHAP
attribution identifies FinBERT sentiment as the dominant feature,
...
content by $t+2$. The $-61\%/+43\%$ negative/positive sentiment asymmetry guides
asymmetric stop-loss design. Adding multilingual (Hindi) sources reduces ECE by 0.9~pp,
...
in reverse (S\&P~500~$\to$~NIFTY, zero-shot) yielded only 53.9\%
(a supplementary observation not included in Table~\ref{tab:transfer}),
echoing the asymmetric information-leadership findings of Kim et al.\ \cite{r11}. We
attribute this to the NSE training distribution encompassing a wider spread of
regimes, producing more robust feature-extractor weights. Re-fitting fewer than 1{,}000 parameters on
...
Granger analysis assumes linearity, and sector-specific NLP fine-tuning has not yet been pursued.
...
trader. Future work will extend to options-market signals, Reddit flows, finance-tuned
decoder LLMs, counterfactual SHAP, and multi-market joint training for
one-shot deployment to new exchanges.
